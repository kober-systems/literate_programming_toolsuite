= Lisi - LIterate Scripting Interpreter
...
===== Snippets aus importierten Dateien nutzen
Wenn Programme größer werden ist es nicht praktisch alles in einer Datei
zu beschreiben. Asciidoc Dokumente lassen sich mit dem `include` macro
importieren. Die Schnipsel in einem importierten Dokument kann man auch
in dem importierenden Dokument nutzen.

Angenommen wir haben das folgende Dokument.

[[unittest_imported_snippets_via_include_input]]
[source, asciidoc, lisi-raw]
[inputs="unittest_imported_snippets_via_include_external_doc"]
[outputs="unittest_imported_snippets_via_include_output"]
[name="snippets_imported_via_include_macro"]
....
This snippet
...
Und das inkludierte Dokument ist

[[unittest_imported_snippets_via_include_external_doc]]
[source, asciidoc, lisi-raw]
.external_document.adoc
....
This is the content of the external doc. It has a snippet, too.
...
In diesem Fall würde folgende Datei erzeugt.

[[unittest_imported_snippets_via_include_output]]
[source, yaml]
.sample.yaml
----
some_category:
...
Manche Tests benötigen weitere Dateien im (simulierten) Dateisystem. Wir
setzen sie folgendermaßen auf.

[[unit-test-setup_inputs-template]]
[source, rust]
.Die Dateiumgebung vorbereiten
----
env.write("{filepath}",
  r#"{expected_content}"#
);
...
let setup_inputs_template = lisi.get_snippet("unit-test-setup_inputs-template").content;
let tests = "";
...
for name in lisi.get_snippet_names() {
  if name.contains("unittest_") && name.contains("_input") {
    ...
    if "inputs" in snippet.attrs {
      let inputs_names = snippet.attrs.inputs;
      for input_name in split(inputs_names, ",") {
        let setup_inputs_template = setup_inputs_template;
        let input_snippet = lisi.get_snippet(input_name);
        setup_inputs_template.replace("{expected_content}", input_snippet.content + "\n");
        setup_inputs_template.replace("{filepath}", input_snippet.attrs.title);
        setup_in += setup_inputs_template + "\n";
      }
    }
    out_template.replace("{setup_inputs}", setup_in);
