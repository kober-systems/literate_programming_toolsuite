...
[source, asciidoc, lisa-raw, outputs="unittest_sample7_output", name="indented_snippets"]
...
[source, asciidoc, lisa-raw, outputs="unittest_sample8_output", name="snippets_with_params"]
....
...
[source, asciidoc, lisa-raw, outputs="unittest_indented_param_snippets_output", name="indented_snippets_with_params"]
....
We have a snippet
        ...
        match snippet_params.get(identifier) {
          Some(param) => match param {
            ...
            ReferenceParam::Reference(param) => {
              ...
              let content = merge_dependencies_inner(ast, snippets, snippet_params.clone());
        ...
        match snippet_params.get(&param_key) {
          Some(param) => match param {
            ...
            ReferenceParam::Reference(param) => {
              ...
              <<extract_snippet_params>>
              let content = merge_dependencies_inner(ast, snippets, snippet_params, element.as_str());
          ...
          None => match snippets.get(identifier) {
            Some(snippet) => {
              if snippet_params.is_empty() {
                ...
                let ast = CodeblockParser::parse(Rule::codeblock, &input).expect("couldn't parse input.");
                let mut snippet_params = snippet_params.clone();
                let content = merge_dependencies_inner(ast, snippets, snippet_params, element.as_str().trim_start());
                indent(&content, indentation, &mut output);
              }
            }
            None => {
