= Lisa - Literate Intuitive Sourcecode Analysis framework
...
pub fn store(&mut self, name: String, snippet: Snippet) {
  ...
  match base {
    Some(base) => { // <2>
      ...
      base.content.push_str(base.join_str.as_str()); // <4>
...
<4> Standardmäßig fügen wir zwischen aneinandergehängten Snippets eine Leerzeile ein. Das kann aber mit `lisa-join` konfiguriert werden.
