...
Der AST wird von einer oder mehreren Erweiterungen modifiziert::
  ...
  * Erweiterungen können über die <<cmd, Kommandozeile>> oder die <<api>>
...
Ein Output Prozessor (Writer) wandelt den AST in eine Ausgabedatei um::
  * Der Writer kann über die <<cmd, Komandozeile>> oder die <<api>> Parameter
...
Jedes Eingabeformat muss zunächst in einen <<ast>> ungewandelt werden um von asciidoctrine verarebitet werden zu können. Auch alle intern unterstützten Dateiformate implementieren die gleiche Schnittstelle.
...
Manchmal will man den <<ast>> verarbeiten und daraus Informationen nutzen (lisa TODO link) verwendet ihn z.B. um daraus Quelltext-Dateien zu extrahieren). Das ermöglicht sehr viele Anwendungsfälle:
