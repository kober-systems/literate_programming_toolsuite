...
impl MarkdownReader {
    ...
    fn convert_events<'a>(&self, input: &'a str) -> Vec<ElementSpan<'a>> {
        ...
        let mut link_definitions: HashMap<String, String> = HashMap::new();
            ...
            match event {
                    ...
                    let element = match tag {
                        ...
                        Tag::CodeBlock(kind) => {
                            ...
                            if let Some(lang) = lang {
                                elem_span.positional_attributes.push(AttributeValue::String("source".to_string()));
                                elem_span.positional_attributes.push(AttributeValue::String(lang));
                            }
                        ...
                        Tag::Image(_link_type, dest_url, title) => {
                            ...
                            if !title_str.is_empty() {
                                elem_span.positional_attributes.push(
                                    AttributeValue::String(title_str)
                                );
                            }
                ...
                Event::Text(text) => {
                    ...
                    if let Some(elem) = stack.last() {
                        if let Element::TypedBlock { kind: BlockType::Listing } = elem.element {
